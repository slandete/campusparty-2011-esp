...
package com.slandete.android.apps.campussched.io;

import static com.slandete.android.apps.campussched.util.ParserUtils.sanitizeId;
import static com.slandete.android.apps.campussched.util.ParserUtils.splitComma;
import static com.slandete.android.apps.campussched.util.ParserUtils.translateTrackIdAlias;
import static com.slandete.android.apps.campussched.util.ParserUtils.AtomTags.ENTRY;
import static org.xmlpull.v1.XmlPullParser.END_DOCUMENT;
import static org.xmlpull.v1.XmlPullParser.START_TAG;

import com.slandete.android.apps.campussched.es.Setup;
import com.slandete.android.apps.campussched.provider.ScheduleContract;
import com.slandete.android.apps.campussched.provider.ScheduleContract.Sessions;
import com.slandete.android.apps.campussched.provider.ScheduleContract.SyncColumns;
import com.slandete.android.apps.campussched.provider.ScheduleDatabase.SessionsSpeakers;
import com.slandete.android.apps.campussched.provider.ScheduleDatabase.SessionsTracks;
import com.slandete.android.apps.campussched.util.Lists;
import com.slandete.android.apps.campussched.util.ParserUtils;
import com.slandete.android.apps.campussched.util.SpreadsheetEntry;
...
//            "EEEE MMM d yyyy h:mma Z", Locale.US);
    ...
    private static final SimpleDateFormat sTimeFormat = new SimpleDateFormat(
    		Setup.DATETIME_FORMAT, Setup.LOCALE);
    ...
    public ArrayList<ContentProviderOperation> parse(XmlPullParser parser, ContentResolver resolver)
        ...
        while ((type = parser.next()) != END_DOCUMENT) {
            if (type == START_TAG && ENTRY.equals(parser.getName())) {
                ...
                /*
                 * XXX: Do not forget to add first empty col (aka item/title in atom)
                 */
    ...
    private static long parseTime(String date, String time) throws HandlerException {
        ...
        //final String composed = String.format("%s 2010 %s -0000", date, time);
        //final String composed = String.format("%s 2010 %s " + Setup.TIME_ZONE, date, time);
        //final String composed = String.format("%s " + Setup.EVENT_YEAR + " %s " + Setup.TIME_ZONE, date, time);
    	final String composed = String.format("%s %s " + Setup.TIME_ZONE, date, time);
